...
import { addTrack as addTrackToDB } from '../services/dbService';
...
const LibraryItemComponent = React.memo(({ item, selected, hasSelection, onToggleSelection, onDragStart, onDropOnFolder, allowDrop, onContextMenu, onNavigate, onDeleteRequest, onAddToPlaylist, onPflTrack, pflTrackId }: LibraryItemComponentProps) => {
        ...
        >
            <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="truncate">
                    ...
                    {item.type !== 'folder' && (
                        <p className="text-sm text-neutral-500 dark:text-neutral-400 truncate">{item.artist}</p>
                    )}
...
const MediaLibrary: React.FC<MediaLibraryProps> = ({ rootFolder, onAddToPlaylist, onAddTracksToLibrary, onAddUrlTrackToLibrary, onRemoveFromLibrary, onRemoveMultipleFromLibrary, onCreateFolder, onMoveItem, onOpenMetadataSettings, onOpenTrackMetadataEditor, onUpdateTrackTags, onUpdateFolderTags, onPflTrack, pflTrackId }) => {
    ...
    const processFiles = async (files: (File | { name: string; blob: Blob })[]) => {
        ...
        for (const file of files) {
            try {
                ...
                const newId = `track-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
                const fileForDb = blob instanceof File ? blob : new File([blob], fileNameWithExt, { type: blob.type });
                await addTrackToDB(newId, fileForDb);
                const track: Track = {
                    id: newId,
                    title,
                    artist,
                ...
                alert(`Failed to process file: ${file.name}. Check console for details.`);
            }
        }
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {contextMenu && (
                <div
                    ref={contextMenuRef}
                    className="absolute z-20 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-md shadow-lg py-1"
